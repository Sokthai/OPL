...
(define (sum-iter sum counter n)
   (if (> counter n)
       sum
       (sum-iter (+ sum counter) (+ counter 1) n)    
  ))
...
(define (sum-squares-recursive n)
  (if (< n 0) 0 (+ (sum-squares-recursive (- n 1)) (* n n))) 
   )
...
(define (sum-square-iter n c)
  (if (< c 0) 0 (+ (sum-square-iter(+ n 1) (- c 1)) (* n n))))
...
(define (sum-alt-squares-iter c q)
  (if (<= c 0) 0 (+ (sum-alt-squares-iter (- c 1) (+ q 2))(* q q))))
