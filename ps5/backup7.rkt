...
;;                   c  (make-sum 'x 0) is 'x
;;                   b  (make-sum 1 2) is 3
;; [augend is 2+]    e  (make-sum 'x 'y 'x) is '(+ x y z)
;;
...
(define (make-product m1 . multiplicandxx)
 ...
 (define (lengthxx elements)
    (if (null? elements) 0
        (+ 1 (length (cdr elements))))) 
  
(cond
    [(null? multiplicand) m1] ;if null, return a1
    [(= (lengthxx multiplicandxx) 1) (cond ;length 1 case
                            [(=number? m1 0) (car multiplicandxx)]
                            [(=number? multiplicandxx 0) m1]
                            [(and (number? m1) (number? (car multiplicandxx))) (* m1 (car multiplicandxx))]
                            [else (list '* m1 (car multiplicandxx))])]
     
    [(> (lengthxx multiplicandxx) 1) (cond ;length 1 case
                            [(=number? m1 0) (append (list '*) (multiplicand multiplicandxx))]
                            [(and (= (lengthxx (multiplicand multiplicandxx)) 1) (number? m1) (number? (car (multiplicand multiplicandxx)))) 
                                        (* m1 (car (multiplicand multiplicandxx)))]
                            [(and (> (length (multiplicand multiplicandxx)) 1) (number? m1) (number? (car (multiplicand multiplicandxx)))) 
                                        (append (list (* m1 (car (multiplicand multiplicandxx)))) (cdr (multiplicand multiplicandxx)))]
                            [else (append (list '* m1)  (multiplicand multiplicandxx))]
                            )
                           ]
  ) 
...
(if (= (length p) 1) (car p)
      (cond
        [(not (pair? (car p))) (cond ;set up a list 
                                   [(and (number? (car p)) (number? (cadr p))) (multiplicand (append (list (list (* (car p) (cadr p)))) (rest (cdr p))))] ;both #
                                   [(and (variable? (car p)) (variable? (cadr p))) (multiplicand (append (list (list (car p) (cadr p))) (rest (cdr p))))] ;both symbol
                                   [(and (number? (car p)) (variable? (cadr p))) (multiplicand (append (list (list (car p) (cadr p))) (rest (cdr p))))] ; first #, second symbol
                                   [(and (variable? (car p)) (number? (cadr p))) (multiplicand (append (list (list (cadr p) (car p))) (rest (cdr p))))])] ;first symbol, second #
        
        [(variable? (cadr p)) (multiplicand (append (list (append (car p) (list (cadr p)))) (rest (cdr p))))] ;if second is symbol

        [(number? (cadr p)) (if (number? (fcar p)) 
                                (multiplicand (append (list (append (list (* (fcar p) (cadr p))) (fcdr p) )) (rest (cdr p))))
                             
                                (multiplicand (append (list (append (list (cadr p)) (car p) )) (rest (cdr p))))   
                                )] 
