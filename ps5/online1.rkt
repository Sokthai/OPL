...
(define (make-sum a1 . augend)
  (if (= (length augend) 0) a1
  (if (= (length augend) 1)
      (let ((augend (car augend)))
        (cond ((=number? a1 0) augend)
              ((=number? augend 0) a1)
...
(define (make-product m1 . multiplicand)
  (if (= (length multiplicand) 0) m1
      (if (= (length multiplicand) 1)
     (let ((multiplicand (car multiplicand)))
       (cond ((=number? m1 1) multiplicand)
             ((=number? multiplicand 1) m1)
             ((and (number? m1) (number? multiplicand))
              (* m1 multiplicand))
             (else
              (list '* m1 multiplicand))))
 
 
      (cons '* (cons m1 multiplicand)))))
